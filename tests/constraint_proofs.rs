...
fn constraint_c1_every_response_enum_receives_unknown_variant() {
    ...
    for name in response_names {
        ...
        let variant_names: Vec<&str> = variants.iter().map(|variant| variant.name().as_str()).collect();
        assert!(
...
fn constraint_c5_finalize_universal_unknowns_idempotent_on_full_lowering() {
    ...
    assert_eq!(count(body_once), 1, "first assemble injects exactly one Unknown");
    assert_eq!(count(body_twice), 1, "second assemble still has exactly one Unknown (idempotent)");
}
...
fn constraint_c5_inject_unknown_into_enum_body_idempotent_under_many_calls() {
    ...
    assert_eq!(variants.len(), 3, "expected exactly one Unknown added regardless of call count");
