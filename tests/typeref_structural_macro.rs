//! `TypeReference` is a derived `StructuralMacroNode`: the `#[shape(...)]`
//! attribute on each variant is the one source of truth for that variant's
//! canonical head, and the derive generates the decode dispatch and the
//! encoder. There is exactly one head per variant — `(Vec T)`, `(Optional T)`,
//! `(Scope T)`, `(Map K V)`, `(Bytes N)`, the bare `Bytes` leaf, the bare
//! scalar leaves, and a bare PascalCase atom for `Plain`.
//!
//! Round-trip is the witness: every variant decodes from its canonical NOTA
//! form and re-encodes to byte-identical text through the derive. And the
//! dropped alias spellings (`Vector`, `Option`, `KeyValue`, `ScopeOf`) are no
//! longer recognised as their wrapper — they error or fall through to `Plain`,
//! pinning that the aliases are gone (Spirit qz6j).

use nota_next::StructuralMacroNode;
use schema_next::{Name, TypeReference};
...
#[test]
fn vector_round_trips_through_the_vec_head() {
    assert_round_trip("(Vec Topic)", TypeReference::Vector(Box::new(plain("Topic"))));
}
...
#[test]
fn scope_round_trips_through_the_scope_head() {
    assert_round_trip(
        "(Scope Topic)",
...
fn map_round_trips_through_the_flat_map_head() {
    assert_round_trip(
        ...
        TypeReference::Map(Box::new(plain("Topic")), Box::new(plain("RecordIdentifier"))),
    );
}
...
fn scalar_leaf_nests_inside_a_grammar_form() {
    assert_round_trip(
        "(Map String Boolean)",
        TypeReference::Map(Box::new(TypeReference::String), Box::new(TypeReference::Boolean)),
    );
}

#[test]
fn dropped_wrapper_aliases_no_longer_parse_as_their_wrapper() {
    // The parenthesised alias spellings are not grammar forms: the derive has
    // no head for them, so they error rather than decoding to the wrapper they
    // once named.
    for alias in [
        "(Vector Topic)",
        "(Option Topic)",
        ...
        let decoded = TypeReference::from_structural_nota(alias);
        assert!(
            decoded.is_err(),
            "dropped alias {alias} must not decode as a wrapper, got {decoded:?}"
        );
    }
}
...
fn dropped_nested_map_payload_no_longer_parses() {
    // The old `(Map (K V))` nested payload is gone; only the flat `(Map K V)`
    // is the Map grammar form now.
...
fn dropped_alias_heads_are_only_bare_declared_names() {
    // As bare atoms the alias spellings are ordinary PascalCase names: they
