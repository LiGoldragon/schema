...
//! type in a collection or option. The surface forms are native NOTA:
//! `[T]`, `{K V}`, and `(Optional T)`. They lower to
...
fn vec_field_lowers_to_vector_reference() {
    let asschema = lower("() () { Service [Text] Cluster [[Service]] }");
...
fn key_value_field_lowers_to_map_reference() {
    let asschema =
        lower("() () { NodeName [Text] NodeProposal [Text] Cluster [{NodeName NodeProposal}] }");
    let fields = struct_fields(&asschema, "Cluster");
...
fn collection_field_and_plain_field_coexist_in_one_struct() {
    let asschema =
        lower("() () { Trust [Text] Service [Text] Cluster [Trust [Service] (Optional Trust)] }");
    let fields = struct_fields(&asschema, "Cluster");
...
fn nested_collections_lower_recursively() {
    ...
    let asschema = lower("() () { Leaf [Text] Key [Text] Nest [{Key [(Optional Leaf)]}] }");
...
fn collection_payload_lowers_in_an_output_variant() {
    ...
    let asschema =
        lower("() ((Projected {NodeName NodeConfig})) { NodeName [Text] NodeConfig [Text] }");
