...
`TrueSchema` is the in-memory semantic schema-in-Rust endpoint produced by
...
`TrueSchema` -> Rust: authored `.schema` deserializes into the typed source value,
the semantic `TrueSchema` value is rkyv-serializable, and Rust is lowered from
that typed value. The text/binary source artifact lives at `SchemaSourceArtifact`.
...
braces are ordered product components: `Topic String`, `Topics (Vector Topic)`,
`Entry { Topic Topics }`, and `Kind [Decision Correction]`. Struct components
are implicit when a type appears once and use `field.Type` only to disambiguate
repeated component types. Newtypes lower as one contained type reference, not
as a one-entry field map. The default parser accepts the strict surface only;
older declaration forms that repeat their own name are not on the production
lowering path.
